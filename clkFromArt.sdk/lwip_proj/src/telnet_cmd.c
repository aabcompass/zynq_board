...
//		}
	...
	{
		instrumentState.curr_ramp = param;
		SendUserIndSCSettingsToSp3();
		char str[] = "Ok\n\r";
		tcp_write(tpcb, str, sizeof(str), 1);
	}
	else if(sscanf(p->payload, TCP_CMD_SLOWCTRL_ALL_CAPA, &param) == 1)
	{
		//debugSettings.current_thr = param;
		if(param == 1)
			instrumentState.curr_capa = 0;
		else if(param == 10)
	...
	else if(sscanf(p->payload, TCP_CMD_SLOWCTRL_ASIC, &param) == 1)
	{
		...
		tcp_write(tpcb, str, sizeof(str), 1);
	...
	else if(sscanf(p->payload, TCP_CMD_SLOWCTRL_ALL_ASIC, &param, &param2, &param3,
			...
			&mapping_1EC[60], &mapping_1EC[61], &mapping_1EC[62], &mapping_1EC[63]) == N_OF_PIXELS_PER_PMT+3)	{
		SetIndSCCurrentLine(param);
		SetIndSCCurrentAsic(param2);
		SetIndSCDac10(param3);
		for(i=0;i<N_OF_PIXELS_PER_PMT;i++) {
			SetIndSCCurrentPixel(i);
			SetIndSCDac7(mapping_1EC[i]);
		}
		char str[] = "Ok\n\r";
		tcp_write(tpcb, str, sizeof(str), 1);
	}
