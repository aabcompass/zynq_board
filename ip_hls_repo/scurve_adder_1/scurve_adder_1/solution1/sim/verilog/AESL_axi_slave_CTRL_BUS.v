...
//------------------------Parameter----------------------
`define TV_IN_N_ADDS "../tv/cdatafile/c.scurve_adder.autotvin_N_ADDS.dat"
...
parameter scurve_adder_continue_addr = 0;
parameter scurve_adder_auto_start_addr = 0;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        N_ADDS_write_data_finish <= 0;
        write_N_ADDS_run_flag <= 0; 
        write_N_ADDS_count = 0;
        count_operate_depth_by_bitwidth_and_depth (N_ADDS_c_bitwidth, N_ADDS_DEPTH, N_ADDS_OPERATE_DEPTH);
    end
    else begin
        ...
        if (AESL_ready_reg === 1) begin
            write_N_ADDS_run_flag <= 1; 
            write_N_ADDS_count = 0;
        end
        if (write_one_N_ADDS_data_done === 1) begin
            write_N_ADDS_count = write_N_ADDS_count + 1;
            if (write_N_ADDS_count == N_ADDS_OPERATE_DEPTH) begin
                write_N_ADDS_run_flag <= 0; 
                N_ADDS_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_N_ADDS
    integer write_N_ADDS_resp;
    integer process_num ;
    integer get_ack;
    ...
    integer i;
    integer j;
    reg [31 : 0] N_ADDS_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = N_ADDS_c_bitwidth;
    process_num = 1;
    ...
    while (1) begin
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_N_ADDS_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write N_ADDS data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (N_ADDS_c_bitwidth < 32) begin
                        N_ADDS_data_tmp_reg = mem_N_ADDS[write_N_ADDS_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < N_ADDS_c_bitwidth) begin
                                N_ADDS_data_tmp_reg[j] = mem_N_ADDS[write_N_ADDS_count][i*32 + j];
                            end
                            else begin
                                N_ADDS_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    write (N_ADDS_data_in_addr + write_N_ADDS_count * four_byte_num * 4 + i * 4, N_ADDS_data_tmp_reg, write_N_ADDS_resp);
                end
                process_busy = 0;
                write_one_N_ADDS_data_done <= 1;
                @(posedge clk);
                write_one_N_ADDS_data_done <= 0;
            end   
            process_1_finish <= 1;
        end
        @(posedge clk);
...
// Read data from file 
initial begin : read_N_ADDS_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [127 : 0] token; 
  reg [127 : 0] token_tmp; 
  //reg [N_ADDS_c_bitwidth - 1 : 0] token_tmp; 
  ...
  count_seperate_factor_by_bitwidth (N_ADDS_c_bitwidth , factor);
  fp = $fopen(`TV_IN_N_ADDS ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_N_ADDS); 
      $finish; 
  end 
  read_token(fp, token); 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < N_ADDS_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
